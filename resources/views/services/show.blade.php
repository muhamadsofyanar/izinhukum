...
@php
    ...
    $defaultMessage = 'Saya ingin konsultasi '.$service->name.' dan membutuhkan informasi persyaratan, estimasi proses, serta penawaran.';
@endphp
        ...
        <div class="service-v21-package-grid">
            @forelse($service->packages as $package)
                <article class="service-v21-package {{ $package->is_popular ? 'is-popular' : '' }}">
                    ...
                    <div class="service-v21-price">@if($package->original_price)<del>Rp{{ number_format($package->original_price, 0, ',', '.') }}</del>@endif<strong>{{ $package->price > 0 ? $package->formattedPrice() : 'Minta penawaran' }}</strong>@if($package->price_suffix && !in_array($package->price_suffix, ['mulai','penawaran'], true))<small>{{ $package->price_suffix }}</small>@endif</div>
