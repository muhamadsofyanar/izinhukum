...
<form action="{{ route('admin.pipeline.sync') }}" method="post">
    @csrf
    <button class="btn btn-outline-primary" type="submit">Sinkronkan permintaan lama</button>
</form>
@endsection
...
<section class="admin-panel mb-3">
    <form class="p-3" method="get" action="{{ route('admin.pipeline.index') }}">
        <div class="row g-2 align-items-end">
            <div class="col-lg-7"><label class="form-label" for="q">Cari lead</label><input class="form-control" id="q" name="q" value="{{ $search }}" placeholder="Nama, nomor, perusahaan, atau layanan"></div>
    ...
    <article class="admin-panel pipeline-card">
        <div class="pipeline-card-head">
            ...
            <div class="pipeline-contact-actions">
                <a class="btn btn-sm btn-primary" href="https://wa.me/{{ preg_replace('/\D/', '', $lead->contact?->phone ?? '') }}" target="_blank" rel="noopener">Buka WhatsApp</a>
                <a class="btn btn-sm btn-outline-primary" href="{{ route('admin.quotes.create', array_filter(['inquiry' => $lead->inquiry_id, 'lead' => $lead->id])) }}">Buat penawaran</a>
        ...
        <form class="form-grid p-3" method="post" action="{{ route('admin.pipeline.update', $lead) }}">
            ...
            <label class="field field-wide"><span>Alasan tidak lanjut</span><input class="form-control" name="lost_reason" value="{{ $lead->lost_reason }}" placeholder="Diisi jika tahap Tidak lanjut"></label>
